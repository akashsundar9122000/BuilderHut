...
export function ProductForm({
  ...
  return (
    /*
     * Two columns once there is room, not one wide one.
     *
     * The form used to sit in a 672px column with most of a 1440px screen
     * empty beside it. The fix is not to stretch that column — a "Name" field
     * a metre wide is harder to use than a narrow one, and the eye has to
     * travel the whole way back for the next label. So the width buys a second
     * column instead: what the product IS on the left, what it costs and looks
     * like on the right.
     */
    <form action={submit} className="flex flex-col gap-5">
          ...
          <Card>
            <CardBody className="flex flex-col gap-5">
              ...
              <Field
                label="Description"
                htmlFor="description"
                hint="What it is, how big, what it's made of."
              >
                <Textarea
                  id="description"
                  name="description"
          ...
          <Card>
            <CardBody className="grid gap-5 sm:grid-cols-2">
              ...
              <Field
                label="SKU"
                htmlFor="sku"
                hint="Optional. Your own reference code."
              >
                <Input
                  id="sku"
                  name="sku"
                  placeholder="CRO-DAISY-05"
              ...
              <Field
                label="Status"
                htmlFor="status"
                hint="Only live products appear on your storefront."
              >
                <select
                  id="status"
                  name="status"
      ...
      {state.error && !state.field ? (
        <p
          role="alert"
          className="text-danger bg-danger-soft rounded-md px-3 py-2 text-sm"
        >
          {state.error}
        </p>
      ) : null}
